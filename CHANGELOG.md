...
**Generated example canvases for direct inspection surfaced a further, still-visible artifact past
the position-guided fix above: exact seed rows alternating with visibly "quilted" gap rows read as
horizontal banding, most obvious on `value_noise` and `voronoi`.**
This is the seed/gap row
alternation itself, not the earlier alignment bug or large-scale-fragmentation problem -- a known,
named limitation in the module's own docstring since it was written ("Wu & Wang's irregular
scatter hides this better but is substantially more complex to implement; not attempted here").

**What changed in the code.**
`sources/synthesis.py` now has two layout mechanisms, chosen per
flavor via `position_guided` (unchanged flag, same three-flavor split as before): a new
**scattered anchor layout** for `value_noise`/`voronoi`/`reaction_diffusion`, and the original
**row-alternating layout**, kept as its own separate, untouched code path (`_encode_rows`/
`_decode_rows`/`_candidate_weights_rows`), for `attractor` only.
The scattered layout replaces
`row % 2` with a deterministic, seed-derived 2-D boolean anchor mask (`_anchor_mask`, `_mask_seed`)
generated by Poisson-disk-style dart-throwing (`MIN_ANCHOR_DISTANCE = 1.1`, empirically the natural
maximal blue-noise packing density on this grid, ~36-39% anchors) -- not confined to whole rows.
Traversal becomes a plain row-major raster scan of the whole canvas, one cell at a time; a gap
cell's only possibly-known neighbors are **above** and **left** (never below or right, since those
aren't resolved yet in raster order) -- simpler than the row design, which needed both above and
below, and no longer needs a mandatory full anchor row 0 to bootstrap it.
`PatchLibrary` gained a
`stride` parameter (overlapping, pixel-shifted candidates instead of only the non-overlapping tile
grid -- the other half of Wu & Wang's technique) plus a vectorized cost computation
(`_candidate_weights_scattered`) to keep a larger candidate pool practical.

**What was measured.**
Direct visual comparison (upscaled PNG, same message, same seed, before
vs. after) confirms the row-banding is gone for both `value_noise` and `voronoi` -- the new canvas
reads as a uniformly fine-grained patchwork throughout, not alternating smooth/quilted stripes.
Full round-trip correctness holds for all four flavors, including payloads spanning multiple mask
periods; new unit tests cover anchor-mask determinism, periodicity, and minimum-spacing.
Full
`core/` suite (204 tests) and lint pass.
The overlapping-candidate half of the technique was tried
across `stride` in `{1, 2, 4, 8}` and **not adopted as the default** -- see
`memory/rejected-ideas.md`'s new entry for the full measurements: no consistent winner across
flavors (each preferred a different stride), while `stride=1` cost roughly 90x `stride=8`'s
wall-clock time.
`DEFAULT_CANDIDATE_STRIDE` ships as `DEFAULT_PATCH_SIZE` (no overlap); the
`stride` machinery stays in `PatchLibrary`, a generically useful capability, not dead code.

**What it means, and what was decided.**
Kept, for the scattered-anchor mechanism on all three
position-guided flavors; `attractor` is untouched.
The row-banding fix came specifically from
*where* anchors sit (2-D scatter vs. whole rows), not from the richer candidate palette -- an
important, separable finding, since the user's initial framing bundled both under "the paper's
technique."
Same "tried, measured, recorded" methodology throughout; see
`docs/superpowers/specs/2026-09-09-irregular-scatter-layout-design.md` for the full design.

**Ported to `web-demo/` the same day.**
`web-demo/src/app/core/synthesis.ts` mirrors the scattered
layout, the anchor mask, and the row-alternating fallback for `attractor` (`prng.ts` gained
`hashBytes`, a raw-bytes FNV-1a variant of the existing `chainSeed`, for deriving the mask seed
from the regenerated texture's own content -- the anchor arrangement itself won't match the Python
side's for the same seed, since `Prng` (mulberry32) differs from NumPy's PCG64, an already-accepted
divergence for this demo).
10 new Karma tests (anchor-mask determinism/density, a fixed tampering
test rewritten to actually locate a gap cell instead of a hardcoded position); all 67 tests and
`ng lint` pass.
Verified live in a real browser (same build-and-serve-statically method as the
position-guided port): all four flavors reveal correctly, and `value_noise`/`voronoi` show the same
banding-free look the Python screenshots do.
