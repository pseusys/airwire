...
The point of this package is to prove the protocol design — and, once
...
one core trick underneath: an **arithmetic coder** run in reverse.
Normally, arithmetic coding
*compresses* a sequence of symbols into fewer bits by spending less on likely symbols and more on
unlikely ones.
Run backwards, the same machinery does the opposite: feed it the (already
...
sentence or a smoothly blending patch of texture.
Run *forwards* over the resulting output, the
...
2. To disguise a hyperslice's ciphertext, `sources/markov.py` starts at a "beginning of sentence"
   ...
   frequencies.
Those frequencies become the arithmetic coder's weights, and the next few bits of
...
3. The chosen word shifts the state forward (the two-word window slides by one) and the process
   repeats.
A special "end of sentence" marker is just another weighted candidate — choosing it
...
One atom always fills exactly one wire chunk, so `pack_hyperchunk` produces
...
Every disguise variant (each Markov language,
...
- `sources/chunking.py` — cuts a message into large "hyperslices", encrypts each one as a single
  ...
  stop-and-wait acknowledgement/retry layer on top.
The header is encrypted (and, optionally, also
  disguised — `header_encoding`); the ack is encrypted; each hyperchunk carries a session-scoped ID
  so an ack can be tied back to the transfer it belongs to.
See the module's docstring for the full
...
- `sources/handshake.py` — establishes a session before any data transfer: an unsigned, plain-TOFU
  ...
  that sender's messages use for the entire conversation.
See
...
- `sources/corpus.py` — downloads and locally caches the plain-text sentence corpora (English and
  ...
  disguise model.
Downloads once per language, on first use; later calls reuse the cached file.
- `sources/data/*.json` — frozen, pre-trained Markov chains (one per language), built from the
  corpora above via `poetry poe train-stego-model` (`scripts/model.py`).
...
- `sources/synthesis.py` — patch-based reversible texture synthesis, adapted from Wu & Wang's
  ...
  hyperslice's own AEAD nonce) -- four registered instances, one per texture flavor.
Visual quality
...
smoke test, not just the unit suite.
The Tatoeba corpus download is cached between runs since it's
