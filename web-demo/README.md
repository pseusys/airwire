...
See [`core/README.md`](../core/README.md#how-the-steganography-works) for
...
to run except a browser.
The real, authoritative implementation is the Python one in `core/`; this
port exists to *demonstrate* the mechanism, not to replace or extend it.
...
- `arithmetic.ts` — line-for-line port of `arithmetic.py`, using `bigint` throughout (the same
  ...
  JS `number` can represent exactly).
One deliberate deviation: `ceilLog2Ratio` computes
...
- `markov.ts` — port of `MarkovEncoding`'s walk, fetching the same frozen per-language model
  (`core/sources/data/markov_<language>.json`) the real Python encoder trains and uses.
Candidate
...
- `textures.ts` — the four procedural texture generators, ported algorithm-for-algorithm from
  ...
  there.
This demo only needs *itself* to regenerate the same texture from the same seed, so
  `prng.ts` uses a much simpler generator instead (mulberry32) — same seed always gives the same
  texture within this page, but not the same texture a real client would produce.
See `prng.ts`'s
...
`MarkovEncoding.encode_atoms`.
The image path was validated end to end in a real browser
...
`npm run build` does the same model sync before building.
The models aren't committed to this
...
all four flavors) — obfuscate a message, then reveal it back.
The image mode's output is a real
